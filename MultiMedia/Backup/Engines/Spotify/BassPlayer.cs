...
namespace SpofityRuntime
{
	public class SpotifyPlayer : CDON.IPlayEngine
    {
		...
		public String RawFind(Song _Song)
		{
       ...
       /// <summary>
		...
		{
		/*	List<Song> Songs = new List<Song>();
			Spotify.Playlist List = Spotify.Playlist.Create(SpotifySession,Link.Create(PlsID));
		...
		public List<MultiMedia.Views.Playlist> Playlists {
			get 
			{
				List<MultiMedia.Views.Playlist>	Playlists = new List<MultiMedia.Views.Playlist>();
				foreach(Spotify.Playlist _Playlist in SpotifySession.PlaylistContainer.CurrentLists)
				{
					MultiMedia.Views.Playlist D =new MultiMedia.Views.Playlist(this,_Playlist.LinkString,Host);
